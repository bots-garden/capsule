...
func DefineDownscaleRevisionDeploymentRoute(router *gin.Engine, functions map[string]models.Function, capsulePath string, httpPortCounter int, workerDomain, reverseProxy, backend string) {
			...
			if len(functions[functionName].Revisions[revisionName].WasmModules) >= 2 {
				...
				for pid, wasmModule := range functions[functionName].Revisions[revisionName].WasmModules {
					...
					if err != nil {
						...
						RemoveUrlRevisionFromReverseProxy(urlToRemove, functionName, revisionName, reverseProxy, backend)
						// trying to remove url from default revision of the function if it exists
						RemoveUrlRevisionFromReverseProxy(urlToRemove, functionName, "default", reverseProxy, backend)
